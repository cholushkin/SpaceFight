#include "ext/mathalgs/mt_base64.h"
...
int Base64Decode( const char* encoded_string, unsigned char* bytes_to_decode ) {
  ...
  while (in_len-- && ( encoded_string[in_] != '=') && IsBase64((unsigned char)encoded_string[in_])) {
    ...
    if (i == 4) {
      ...
      for (i = 0; (i < 3); i++)
        bytes_to_decode[pos++] = char_array_3[i];
      i = 0;
    }
  }
  ...
  if(i) {
    ...
    for (j = 0; (j < i - 1); j++)
      bytes_to_decode[pos++] = char_array_3[j];
  }
  return pos;
}
